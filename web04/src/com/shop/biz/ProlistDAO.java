...
public class ProlistDAO {
	...
	public ArrayList<ProlistVO> getProlistList() {
		...
		try {
			...
			String sql = "select prolistid, pcode, pname, nat, genre, price, img from Prolist order by prolistid";
	...
	public int updateProlist(ProlistVO vo) {
		...
		try {
			conn= DBConn.getConnection();
			String sql = "update Prolist set pcode=? pname=? nat=? genre=? price=? img=? where Prolistid=?";
...
//여기부터 시작
	public int insertProlist(ProlistVO vo) {
		int r=0;
		try {
			conn = DBConn.getConnection();
